...
import { getOpenRouterApiKey, getOpenRouterModel } from "./../index";
 ...
 * OpenRouter uses OpenAI-compatible API but requires additional headers
 */
...
async function fetchLLMTaskSummarizer(
  ...
  const personalAIApiKey = await getOpenRouterApiKey();
  if (!personalAIApiKey) return;

  const model = (await getOpenRouterModel()) || DEFAULT_AI_MODELS.openrouter;
  ...
  try {
    const response = await fetch(aIApiUrl, {
      method: "POST",
      headers: buildOpenRouterHeaders(personalAIApiKey),
      body: JSON.stringify({
        model: model,
        messages: messages,
        stream: true,
...
async function invokingCodeAnalysis(issueDetails: any, diffsData: any) {
  ...
  try {
    ...
    mainContainer.appendChild(createErrorContainer("OpenRouter"));
  }
}
...
async function invokingMRAction(containerRef: any, diffsData: any, actionType: MRActionType) {
  ...
  try {
    ...
    MRActionsRenderer.showErrorState(mainContainer, "Failed to get response from OpenRouter. Please try again.");
  }
}
...
async function invokingIssueAction(containerRef: any, issueData: any, discussions: any, actionType: IssueActionType) {
  ...
  try {
    ...
    IssueActionsRenderer.showErrorState(mainContainer, "Failed to get response from OpenRouter. Please try again.");
  }
}
...
async function invokingIssueChat(
  ...
  try {
    ...
    IssueChatRenderer.showErrorState(messageContainer, "Failed to get response from OpenRouter. Please try again.");
  }
}
